...
term is as follows
\begin{eqnarray*}
cov(E[Z_{i}\vert S_{i}],E[Z_{i+k}\vert S_{i+k}]) & = & (\text{\ensuremath{\boldsymbol{\mu}_{Z\vert S}}}-\mu_{Z}\boldsymbol{1})'\text{diag}(\pi_{S})P^{k}(\boldsymbol{\mu}_{Z\vert S}-\mu_{Z}\boldsymbol{1}),
\end{eqnarray*}
