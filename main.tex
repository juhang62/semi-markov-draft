...
from this transition (Figure \ref{fig:diagram}). If we assume the transition from $S_{i}\to S_{i+1}$ follows
...
step sizes.{\bf JF: Also, maybe look for other people's similar approaches to multiple motors. LF: tried keywords: motor CLT. but didn't find any} Here we extend this method to a group of motors.
...
Second moment is less often studied in experimental studies, probably because it is expensive to replicate the same experiment many times.\emph{JF: Why? LF: is this explanation ok? } In Figure \ref{fig:M2},
