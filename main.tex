...
\maketitle
\begin{abstract}
We propose a general approach to study cellular transport driven by a group
...
In this section, we review a previously established framework for molecular motor models that break each kinetic cycle into a time and a displacement \cite{Hughes2011,Hughes2012}, and then use central limit theory to establish asymptotic quantities typically measured in experiments.  The goal of this paper is to generalize the technique to motor-cargo complexes.
...
kinetic states of individual motors, then $\tau_{i}$ is an exponential
random variable. {\bf JF: I don't quite understand the following two sentences.}  In general, neither the state space of $\{S_{i}\}$
needs to be the set of all possible group states and nor does the transition
...
\begin{enumerate}
\item $S_{i}$ is a aperiodic discrete Markov chain and has a stationary distribution on a finite state space.
\item the first two moments of $Z_i \vert S_{i-1}$, $\tau_i \vert S_{i-1}$ and the first moment of $Z_{i}\vert S_{i},S_{i-1}$ and $\tau_{i}\vert S_{i},S_{i-1}$ are finite and can be computed.
\item the dependence between variables are indicated by the graph in Figure \ref{fig:diagram}, particularly we need the Markov property $P(Z_i,\tau_i | \{ S_j \}_{j\le i} )=P(Z_i,\tau_i | S_{i-1}, S_i )$ and conditional independence $P(Z_i,Z_{j} | S_i) = P(Z_i | S_i) P(Z_{j} | S_i)$ for $j>i$ and similarly for $\tau_i$.
\end{enumerate}
...
is 0 if $S_{i-1} \to S_{i}$ does not involve any motor stepping  and $1/m$ otherwise, where $m$ is the number of motors in the group. \emph{Are we not allowing the multiple chemical steps?  Why not wait until a physical step happens?}
...
Second moment is less often studied in experimental studies.\emph{JF: Why?} In Figure \ref{fig:M2},
