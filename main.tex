...
In this section, we review a previously established framework for molecular motor models that summarizes each kinetic cycle as a time and a displacement \cite{Hughes2011,Hughes2012}, and then use a central limit theorem (CLT) to establish asymptotic quantities for position typically measured in experiments.  The goal of this paper is to generalize these technique to motor-cargo complexes.

In order to proceed, we will need to define a cycle somewhat abstractly and then fill in details with specific examples.  The central limit theorem for the position process involve sums of both the change in position of the motor-cargo complex during the $i$th cycle, which we will denote $Z_i$, as well as the time of the $i$th cycle, which we will denote $\tau_i$.  We will also need to carry some auxiliary information from one cycle to the next which we denote with $S_i$; one important aspect of the methods presented here is that $S_i$ will form a Markov chain irrespective of $Z_i$ and $\tau_i$.

The Markov chain $\{S_{i}\}_{i\in\mathbb{N}}$ where $S_{i}$
represents the relative position and the kinetic state (together called the ``group state'' henceforth) of each motor at ``time'' $i$. We
...
results in a physical step. \emph{JF: I don't think we want to set it up so that S is always the combined chemical state and relative position.  A better description is the chemical state and relative position of the system WHEN a physical transition occurs.Right? Or at least we say--that in each example we will make as the definition of a cycle either a chemical transition or a physical transition. } Let $\tau_{i}$ be the time for the transition of $S_{i}\to S_{i+1}$ to happen and $Z_{i}$ be the change in the group position defined as the arithmetic average of positions of all motors resulting
from this transition (Figure \ref{fig:diagram}). If we assume the transition from $S_{i}\to S_{i+1}$ follows every change in the group state due to a transition in the underlying kinetic states of individual motors, then $\tau_{i}$ is an exponential random variable. In general, neither the state space of $\{S_{i}\}$ needs to be the set of all possible group states nor does the transition of $S_{i}\to S_{i+1}$ to exactly follow every change in the group state due to the individual motors' kinetic transitions. \emph{JF: this previous sentence contrdicts the first sentence of the paragraph.} Being aware of this sometimes would help simplify calculation as we will see later. 
...
Let $X(t)$ be the position of the motor-cargo complex at time $t$. Using the variables defined above, we can write $X(t)=\sum_{i=1}^{N(t)}Z_{i}$ where $N(t)=\inf\{n\in\mathbb{N}:\sum_{i=1}^{n}\tau_{i}\ge t\}$. Two quantities commonly observed in experiments and studied theoretically
are asymptotic speed and diffusivity, i.e., $V=\lim_{t\to\infty}\frac{E[X(t)]}{t}$
and $D=\lim_{t\to\infty}\frac{\text{Var}[X(t)]}{2t}$. Since many experimental observations of motors occur at significantly long time and distance in contrast to microscopic motor dynamics, from a CLT (central limit theory) point of view, the position process will appear essentially like a Brownian motion. To make it precise, we want to cast a multi-motor model into a framework of semi-Markov processes so that these two asymptotic quantities of interests can be easily calculated from a functional CLT. Towards this goal, we consider
...
\item The first two moments of $(Z_i, \tau_i)'$ given $S_{i}$ and $S_{i+1}$, are finite and can be computed.
\item the dependence between variables are indicated by the graph in Figure \ref{fig:diagram}, particularly we need the Markov property $p(Z_i=z,\tau_i=t | \{ S_j \}_{j\le i} )=p(Z_i=z,\tau_i=t | S_{i})$ and conditional independence $p(Z_i=z,Z_{j}=r | S_i) = p(Z_i=z | S_i) p(Z_{j}=r | S_i)$ for $j>i$ and similarly for $\tau_i$.
\end{enumerate}
...
is 0 if $S_{i} \to S_{i+1}$ does not involve any motor stepping  and $1/m$ otherwise, where $m$ is the number of motors in the group. \emph{Are we not allowing the multiple chemical steps?  Why not wait until a physical step happens? LF: We can as how it is done in the next example. Here, I didn't allow multiple chemical steps at the time when the strong assumptions for the covariance calculation was not relaxed. Now it is not necessary to be handled like this}
...
grows factorially (exponentially) with number of motors. In experimental studies such as the DNA origami experiments described in the introduction,
...
of the group behavior. Since the kinetic model is simple, we can come up with two parameter sets that have same first moment (force-velocity curve) of 1-motor system. The parameter values are summarized in Table \ref{tab:parameters} under the name "generic kinesin".  As we can see for 2-motor system, the first parameter set($k_{ab}=500$) differs from 
...
in increase in velocity. As in Figure \ref{fig:fcomp surf}, our results
show that asymmetry in detachment rate plays only a marginal role while $k_{det}$ shifts the surface in a larger range. It should
...
\caption{\label{fig:FV 1-2-motor}force-velocity curves of a single motor, 2-motor complex with and without detachment (deprecated? )}
\end{figure}
...
\caption{\label{fig:fcomp surf} surface plot of compensation force vs $k_{att}$ and $\delta_{4}$. Left: $k_{det}=5$. Right: $k_{det}=20$}
\end{figure}
...
of quasi-stationary approximation. In Figure \ref{fig:qss-FVFD}, force-velocity
