...
represents the relative position and certain form of the kinetic state of each motor (together called the ``group state'' henceforth)  at ``time'' $i$. Note that how exactly the kinetic states are represented by $S_{i}$ should be decided carefully case by case as we will illustrate later by examples.   We
...
biological relevant parameters found in the literature shown in Table \ref{tab:parameters}.
...
would be slowed down by the drag caused by the trailing motor while
...
\includegraphics[width=.5\textwidth]{figures/abc/500vs5000newest/force-dif-good}
...
Moreover, our framework made it simple to generates stationary distribution of
