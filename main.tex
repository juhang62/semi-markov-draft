...
represents the relative position and certain form of the kinetic state of each motor (together called the ``group state'' henceforth)  at ``time'' $i$. Note the how exactly the kinetic states are represented by $S_{i}$ should be decided carefully case by case as we will illustrate later by examples.   We
...
a motor changes its kinetic state, the relative position only changes if this transition results in a physical step. \emph{JF: I don't think we want to set it up so that S is always the combined chemical state and relative position.  A better description is the chemical state and relative position of the system WHEN a physical transition occurs.Right? Or at least we say--that in each example we will make as the definition of a cycle either a chemical transition or a physical transition. LH: I have rewritten it. Let me if it is clear now} Let $\tau_{i}$ be the time for the transition of $S_{i-1}\to S_{i}$ to happen and $Z_{i}$ be the change in the group position defined as the arithmetic average of positions of all motors resulting
